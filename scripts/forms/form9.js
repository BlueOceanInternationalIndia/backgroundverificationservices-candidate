...
const form9 = {
    onLoad: async () => {
        ...
        const resp = await axios.post(`${DATA_SERVER_URI}/candidate/personalreferences/${activeUser.uid}`, { token: aTr }).then(async (res) => {
    ...
    onAction: async (e) => {
        if(e.target.id == 'DeleteEntry') {
            ...
            const resp = await axios.delete(`${DATA_SERVER_URI}/candidate/personalreferences/${entryId}`, { data: { token: aTr } }).catch((err) => console.log('Failed To Delete Entry. Error: ', err))
            return true
        }
    },
    ...
    onSubmit: async () => {
        ...
        const resp = await axios.post(`${DATA_SERVER_URI}/candidate/personalreferences`, userData).then((res) => {
            activeUser.log.form9.submitted = true;
            return true;
